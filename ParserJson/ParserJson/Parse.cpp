...
bool Parse::is_whitespace(const char c) {
    return isspace(c);
}
int Parse::next_whitespace(const string& source, int i) {
...
int Parse::skip_whitespaces(const string& source, int i) {
    while (i < (int)source.length()) {
        if (!is_whitespace(source[i])) return i;
        i++;
    }
...
vector<Parse::token> Parse::tokenize(string source) {
    source += " ";
    vector<token> tokens;
...
JsonValue Parse::json_parse(vector<token> v, int i, int& r) {
    JsonValue current;
    if (v[i].type == CROUSH_OPEN) {
        current.set_type(JsonType::JOBJECT);
        int k = i + 1;
        while (v[k].type != CROUSH_CLOSE) {
            string key = v[k].value;
            k += 2; // k+1 should be ':'
            int j = k;
            JsonValue vv = json_parse(v, k, j);
    ...
    if (v[i].type == BRACKET_OPEN) {
        current.set_type(JsonType::JARRAY);
        int k = i + 1;
    ...
    if (v[i].type == NUMBER) {
        current.set_type(JsonType::JNUMBER);
        current.set_string(v[i].value);
        r = i + 1;
        return current;
    }
    if (v[i].type == STRING) {
        current.set_type(JsonType::JSTRING);
        current.set_string(v[i].value);
        r = i + 1;
        return current;
    }
    if (v[i].type == BOOLEAN) {
        current.set_type(JsonType::JBOOLEAN);
        current.set_string(v[i].value);
        r = i + 1;
        return current;
    }
    if (v[i].type == NUL) {
        current.set_type(JsonType::JNULL);
...
JsonValue Parse::parse(const string& str) {
    int k;
    return json_parse(tokenize(str), 0, k);
}
JsonValue Parse::parse_file(const string& filename) {
